...
async function main() {
  ...
  try {
    ...
    const courseId = await page.evaluate(() => {
      return document
        .querySelector("body[data-clp-course-id]")
...
async function processLecture(
  ...
  try {
    ...
    if (downloadSrt) {
      ...
      try {
        // Extract all cues (timestamp + text) at once from the transcript panel
        const cues = await page.evaluate(() => {
          ...
          const items = Array.from(
            panel.querySelectorAll('[data-purpose="transcript-cue"]')
          );
          return items.map((el) => {
            const full = el.textContent.trim();
            const m = full.match(/^(\d{1,2}:\d{2}(?::\d{2})?)[\s\n]+([\s\S]+)/);
            if (m) {
              return { start: m[1], text: m[2].trim() };
            } else {
              return { start: null, text: full };
            }
          });
        });

        if (!cues.length) {
          console.log(`No transcript cues found for: ${sanitizedFilename}`);
        } else {
          // Determine end times for each cue
          const srtData = cues.map((cue, i) => {
            const startFormatted = toSrtTimestamp(cue.start || "0:00");
            const endRaw =
              i < cues.length - 1 ? cues[i + 1].start : cues[i].start;
            const endFormatted = toSrtTimestamp(endRaw || cue.start || "0:00");
            return {
              index: i + 1,
          ...
          const srtFile = path.join(
            __dirname,
            "../output",
            `${sanitizedFilename}.srt`
          );
          const srtLines = srtData
            .map((c) => `${c.index}\n${c.start} --> ${c.end}\n${c.text}\n`)
            .join("\n");
          fs.writeFileSync(srtFile, srtLines, "utf8");
          console.log(`SRT file saved for: ${sanitizedFilename}`);
        }
      } catch (err) {
        console.log(
          `Error generating SRT for ${sanitizedFilename}: ${err.message}`
        );
      }
    }
